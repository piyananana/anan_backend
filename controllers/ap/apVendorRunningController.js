// controllers/ap/apVendorRunningController.js

const formatVendorCode = (config) => {
...
// GET /ap_vendor_running
const fetchConfig = async (req, res) => {
  try {
    const result = await req.dbPool.query(
      `SELECT * FROM ap_vendor_running ORDER BY id LIMIT 1`
    );
    ...
    console.error('Error fetching ap_vendor_running:', error);
    res.status(500).json({ message: 'Internal server error' });
  }
};

// GET /ap_vendor_running/preview_code
const previewCode = async (req, res) => {
  try {
    ...
    res.status(200).json({ vendor_code: formatVendorCode(config) });
  } catch (error) {
    console.error('Error previewing vendor code:', error);
    res.status(500).json({ message: 'Internal server error' });
  }
};

// POST /ap_vendor_running (upsert)
const saveConfig = async (req, res) => {
  ...
  try {
    const existing = await req.dbPool.query(`SELECT id FROM ap_vendor_running LIMIT 1`);
    let result;
    if (existing.rows.length === 0) {
      result = await req.dbPool.query(
        `INSERT INTO ap_vendor_running
           (is_auto_numbering, format_prefix, format_separator,
            format_suffix_date, running_length, next_running_number,
    ...
    console.error('Error saving ap_vendor_running:', error);
    res.status(500).json({ message: 'Internal server error' });
  }
};

// สำหรับใช้ภายใน apVendorController (atomic increment ภายใน transaction)
const generateNextCode = async (client) => {
  const result = await client.query(
    `SELECT * FROM ap_vendor_running ORDER BY id LIMIT 1 FOR UPDATE`
  );
  if (result.rows.length === 0 || !result.rows[0].is_auto_numbering) return null;
  const config = result.rows[0];
  const code = formatVendorCode(config);
  await client.query(
    `UPDATE ap_vendor_running SET next_running_number = next_running_number + 1 WHERE id = $1`,
    [config.id]
  );
  return code;
};
