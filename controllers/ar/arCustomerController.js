  ...
  try {
    ...
    let query = `
      SELECT
        ...
        c.currency_code, c.is_active, c.requires_billing, c.is_code_auto_generated,
...
const updateRow = async (req, res) => {
  ...
  try {
    ...
    const newGroupId = customer_group_id || null;
    const currentGroupId = current.customer_group_id || null;
    if (current.is_code_auto_generated) {
      if (newGroupId !== currentGroupId) {
        await client.query('ROLLBACK');
        return res.status(400).json({ message: 'ไม่สามารถเปลี่ยนกลุ่มลูกหนี้ได้ เนื่องจากรหัสลูกหนี้นี้ถูกออกโดยอัตโนมัติตอนสร้างข้อมูล' });
      }
    } else if (newGroupId !== null && newGroupId !== currentGroupId) {
      const groupResult = await client.query(`SELECT is_auto_number FROM ar_customer_group WHERE id = $1`, [newGroupId]);
      if (groupResult.rows.length > 0 && groupResult.rows[0].is_auto_number) {
        await client.query('ROLLBACK');
        return res.status(400).json({ message: 'ไม่สามารถเปลี่ยนไปยังกลุ่มที่ตั้งค่ารหัสอัตโนมัติได้ เนื่องจากรหัสลูกหนี้นี้ป้อนเอง' });
      }
    }
