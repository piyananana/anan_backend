...
const { ensureImItemCategoryTable, generateNextCodeForCategory } = require('./imItemCategoryController');
...
const addRow = async (req, res) => {
    ...
    try {
        ...
        const codeWasProvided = !!(b.item_code && b.item_code.trim() !== '');
        let finalCode = codeWasProvided ? b.item_code.trim().toUpperCase() : null;
        // ลำดับความสำคัญ: ระบุเอง -> รหัสอัตโนมัติของหมวดหมู่ (ถ้ามี) -> รหัสอัตโนมัติส่วนกลาง
        if (!finalCode && b.category_id) {
            finalCode = await generateNextCodeForCategory(client, b.category_id);
        }
        if (!finalCode) {
        ...
        const result = await client.query(
            ...
            [
                ...
                b.is_active ?? true, isCodeAutoGenerated, userName,
            ]
        );
...
const updateRow = async (req, res) => {
    ...
    try {
        ...
        const currentResult = await client.query(
            `SELECT category_id, is_code_auto_generated FROM im_item WHERE id = $1`, [id]
        );
        ...
        const newCategoryId = b.category_id || null;
        const currentCategoryId = current.category_id || null;
        if (current.is_code_auto_generated) {
            if (newCategoryId !== currentCategoryId) {
                await client.query('ROLLBACK');
                return res.status(400).json({ message: 'ไม่สามารถเปลี่ยนหมวดหมู่สินค้าได้ เนื่องจากรหัสสินค้านี้ถูกออกโดยอัตโนมัติตอนสร้างข้อมูล' });
            }
        } else if (newCategoryId !== null && newCategoryId !== currentCategoryId) {
            const categoryResult = await client.query(`SELECT is_auto_number FROM im_item_category WHERE id = $1`, [newCategoryId]);
            if (categoryResult.rows.length > 0 && categoryResult.rows[0].is_auto_number) {
                await client.query('ROLLBACK');
                return res.status(400).json({ message: 'ไม่สามารถเปลี่ยนไปยังหมวดหมู่ที่ตั้งค่ารหัสอัตโนมัติได้ เนื่องจากรหัสสินค้านี้ป้อนเอง' });
            }
        }
